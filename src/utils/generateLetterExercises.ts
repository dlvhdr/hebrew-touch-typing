...
const getEmptyLinesArray = (): undefined[] => {
  return new Array(LESSON_NUM_LINES)
    .fill(undefined)
};
...
export const generateLetterReviewExercise = (lessonLetters: LessonNewLetters): ExerciseText => {
  return getEmptyLinesArray()
    .map(_line => fillLinesWithRandomLetters(lessonLetters));
}

// Practice lesson should contain all the letters you've learned so far
export const generateLetterPracticeExercise = (exerciseNumber: number): ExerciseText => {
  const letters = letterLearningOrder.slice(0, exerciseNumber + 1).flat();
  return getEmptyLinesArray()
    .map(_line => fillLinesWithRandomLetters(letters));
}
...
export const getFullListOfPracticeAndReviewExercises = (): Exercise[] => {
  return letterLearningOrder.map(( newLetters: LessonNewLetters, exerciseNumber: number ) => {
    return [
      { text: generateLetterReviewExercise(newLetters), type: ExerciseType.REVIEW },
      { text: generateLetterPracticeExercise(exerciseNumber), type: ExerciseType.PRACTICE },
    ];
  }).flat();
}